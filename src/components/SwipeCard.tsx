      ...
      <div className="relative h-full">
        ...
        {allImages.length > 1 && (
          <div className="absolute inset-x-0 top-0 h-full flex">
            <div className="w-1/2 h-full cursor-pointer" onClick={prevImage}></div>
            <div className="w-1/2 h-full cursor-pointer" onClick={nextImage}></div>
          </div>
        )}
        ...
        <div
          className={cn(
            "absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent p-6 text-white transition-all duration-300 cursor-pointer",
        ...
        >
          ...
          {/* Tap to expand hint */}
          {!showDetails && <div className="mt-2 text-white/60 text-xs">Tap to see more details</div>}
        </div>
      </div>
    </div>
  )
