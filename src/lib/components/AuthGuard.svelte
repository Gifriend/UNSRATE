<script lang="ts">
  ...
  import { PROTECTED_ROUTES, PUBLIC_ROUTES, AUTH_REDIRECT, LOGIN_REDIRECT } from '$lib/stores/auth';
  ...
  const isPublicOnlyRoute = $derived(
  ...
  const showLoading = $derived(auth.isLoading);
  const showContent = $derived(
    !auth.isLoading && (
      (auth.isAuthenticated && !isPublicOnlyRoute) ||
      (!auth.isAuthenticated && !isProtectedRoute) ||
      (auth.isAuthenticated && isPublicOnlyRoute)
    )
  );
</script>
